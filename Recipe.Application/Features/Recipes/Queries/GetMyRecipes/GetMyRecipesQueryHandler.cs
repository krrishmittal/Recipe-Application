...
/// <summary>
/// Handles the get my recipes query.
/// </summary>
public class GetMyRecipesQueryHandler : IRequestHandler<GetMyRecipesQuery, ApiResponse<PagedResponse<RecipeResponse>>>
{
    private readonly RecipeDbContext _db;
    private readonly ILogger<GetMyRecipesQueryHandler> _logger;

    /// <summary>
    /// Initializes a new instance of the GetMyRecipesQueryHandler class.
    /// </summary>
    public GetMyRecipesQueryHandler(
        RecipeDbContext db,
        ILogger<GetMyRecipesQueryHandler> logger)
    {
        _db = db;
        _logger = logger;
    }
    ...
    /// </summary>
    public async Task<ApiResponse<PagedResponse<RecipeResponse>>> Handle(GetMyRecipesQuery request, CancellationToken cancellationToken)
    {
        try
        {
        ...
        {
            _logger.LogError(ex, "Unexpected error in {Handler}", nameof(GetMyRecipesQueryHandler));
            return ApiResponse<PagedResponse<RecipeResponse>>.Fail("User recipe listing failed due to an unexpected error.", 500, nameof(GetMyRecipesQuery));
        }
    }
